...
/// Connect: `GET /github/oauth?redirectUrl=<encoded>` + Bearer → deep link → `GET /github/me`
...
class GithubOAuthService {
  ...
  Future<String> _fetchLoginAuthorizeUrl(String redirectUri) async {
    ...
    try {
      ...
      if (e.response?.statusCode == 404) {
        ...
        throw ApiException('API không hỗ trợ POST /auth/github cho đăng nhập GitHub.');
  ...
  Future<void> connectWithJwt(Dio authenticatedDio) async {
    ...
    final rawUrl = extractOAuthAuthorizeUrl(payload) ?? extractOAuthAuthorizeUrl(res.data);
    ...
    final meRes = await authenticatedDio.get<dynamic>('/github/me');
    ...
    if (account is! Map || account.isEmpty) {
      ...
      throw ApiException('Kết nối GitHub chưa xác nhận được — GET /github/me trống');
